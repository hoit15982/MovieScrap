			...
			<form action="join_ok" method="post" onSubmit="return frmJoinSubmit(this);">
				...
				<table class="tbl_join">
						...
						<c:choose>
							<c:when test="${ss_is_login }">
								<td><input type="hidden" name="mb_name"
									value="${member.mb_name }" /> ${member.mb_name }</td>
							</c:when>
							<c:otherwise>
						...
						<c:choose>
							<c:when test="${ss_is_login }">
								<td><input type="hidden" name="mb_id" size="80"
									value="${member.mb_id }" /> ${member.mb_id }</td>
							</c:when>
							<c:otherwise>
								<td><input type="text" name="mb_id" size="80" value="${member.mb_id }" /> <a
					...
					<tr>
						<th>이메일</th>
						<td>
						<input type="text" name="mb_email" value="${member.mb_email }" size="100" placeholder="이메일을 정확하게 입력해주세요." />&nbsp; 
						
						<span class="chk_area <c:if test="${member.mb_mailing  == 1}"> on </c:if> ">
						<label for="mb_mailing"><input type="checkbox" id="mb_mailing" <c:if test="${member.mb_mailing == 1}"> checked=checked </c:if> name="mb_mailing" value="1" />
						</label> 메일링여부</span>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td><input type="text" name="mb_tel"
							value="${member.mb_tel }" placeholder="숫자만 입력해주세요." />
						</td>
					</tr>
					<tr>
						<th>핸드폰번호</th>
						<td><input type="text" name="mb_hp" value="${member.mb_hp }"
				...
				<div class="btn_area_right">
					...
					<a href="${PATH}/index.do" class="btn02">취소</a>
				</div>
			</form>
		</div>
	</div>
